...
This repo contains ZSH + bash scripts for RF capture (video8 / 8mm) at 40MHz/40MSPS via the vhs-decode project for the follwing operations:

1. CX auto level calibration, zsh script
   #+begin_src
source cx-calibrate.zsh
   #+end_src
2. FLAC RF capture, zsh script
   #+begin_src
zsh capture-rf-flac.zsh /path/to/capture-40msps-8bit-cx-card.flac
   #+end_src
